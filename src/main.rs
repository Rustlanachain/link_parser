...
"#;
    ...
    loop {
        ...
        println!("Enter 1-4 to parse different HTML docs, <any other number> to quit:");
        io::stdin().read_line(&mut user_input)?;
        let user_input = user_input.trim_end();

        match user_input.parse::<usize>()? {
            1 => {
                let result = parser::generate_links(html_doc1);
                println!("{:?}", result);
            }
            2 => {
                let result = parser::generate_links(html_doc2);
                println!("{:?}", result);
            }
            3 => {
                let result = parser::generate_links(html_doc3);
                println!("{:?}", result);
            }
            4 => {
                let result = parser::generate_links(html_doc4);
                println!("{:?}", result);
            }
            _ => {
